...
import { z } from "zod";
import {
  buildCopyFromStrategy,
  buildProfessionalLayers,
  layersFromAiOrStrategy,
  pickProTemplate,
} from "@/lib/image/professional-style";
import {
  createTextLayer,
  normalizeLayer,
  type TextLayer,
...
function rawLayersToTextLayers(
  raw: z.infer<typeof creativeDesignSchema>["layers"]
): TextLayer[] {
  return raw.map((layer, index) =>
    normalizeLayer(
      createTextLayer(applyLocalSpanishFixes(truncate(layer.text.replace(/\n/g, " "), 48)), {
        y: 14 + index * 18,
        fontSize: index === 1 ? 56 : 28,
...
function toProfessionalLayers(
  ...
  return layersFromAiOrStrategy(aiLayers, { ...params, strategy });
}
...
export async function suggestImageTextLayers(params: {
  ...
  try {
    ...
    const textOnly = rawLayersToTextLayers(design.layers);
    if (textOnly.length < 3) return buildSmartFallback(params, direction, strategy);
    const proofread = await proofreadLayerTexts(textOnly, params.aiBrief);
    return toProfessionalLayers(proofread, params, strategy);
  } catch {
    return buildSmartFallback(params, direction, strategy);
  }
}
...
/** Fallback con plantilla profesional de agencia */
function buildSmartFallback(
  ...
  _direction: CreativeDirection,
  strategy: MarketingStrategy | null
): TextLayer[] {
  const template = pickProTemplate(params.aiBrief, params.discountPercent);
  const copy = buildCopyFromStrategy({ ...params, strategy });
